...
namespace Obfuscator.Obfuscation.Generation
{
    ...
    public class ScriptBuilder
    {
        ...
        { if (!ObfuscationSettings.EnhancedOutput) { return (""); }; return ("PSU_LUA_GENERATION"); }
...
local Mode = {ToExpression("#", "String")}; local function _R(...) return ({{...}}), Select(Mode, ...); end;";
        ...
        public string BuildScript(string Location)
        {
            ...
            string Source = "return (function(T, ...) local TEXT = \"This file was obfuscated using ALPHA Obfuscator 4.0.A | https://www.google.com/ & discord.gg\"; " + Variables + Functions + Deserializer + VM;
            Source += $" \nend)(({{{VMTable}}}), ...);";
            ...
            string Options = "--maximum --opt-entropy --opt-emptylines --opt-eols --opt-numbers --opt-whitespace --opt-locals --noopt-strings \"" + Path.Combine(dir,Location, "VM.lua") + "\" -o \"" + Path.Combine(dir,Location, "Output.lua") + "\"";
            ...
            {
                StartInfo.FileName = "luasrcdiet";
                StartInfo.Arguments = Options;
            } else
            {
                StartInfo.WorkingDirectory = "Lua/lua";
                StartInfo.FileName = Path.Combine(dir, $"Lua/LuaJIT.exe");
                StartInfo.Arguments = Path.Combine(LuaDir, $"LuaSrcDiet.lua") + Options;
            }
