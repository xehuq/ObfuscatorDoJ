...
namespace DiscordBot
{
    public class Program
    {
        ...
        public static ulong EPIC = 451415338803003393;
        public static bool RESTRICTED_MODE = false;
        public static string VERSION = "4.3.A";
        ...
        //private static string Token = "NzcyMjczNzAyODA0NzgzMTM0.X54R7g.u1kTDeQUIBMg8iAGs5N5RwTXduA";
        ...
        public async Task RunBotAsync()
        {
            ...
            await Client.SetGameAsync("discord.gg, the #1 Lua Obfuscator", null, ActivityType.Playing);
            Console.WriteLine("Started");
            Console.WriteLine(Client.Rest.CurrentUser.Id);
        ...
        private async Task HandleCommandAsync(SocketMessage Argument)
        {
            ...
            else if (Processes.ContainsKey(UserId))
            {
                ...
                async void StartObfuscationProcess()
                {
                    ...
                    try { File.AppendAllText(@"Storage\ObfuscationHistory", $"{Message.Timestamp}: {Message.Author.Username}#{Message.Author.Discriminator} (ID: {Message.Author.Id}) Obfuscated a File @ {Message.Timestamp} ({Content.Length} Bytes)\n"); } catch (Exception Exception) { Console.WriteLine(Exception); Error("Unknown Error #1"); return; };
